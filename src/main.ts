...
export default class Md2JiraPlugin extends Plugin {
	...
	convertToJiraFormat(content: string): string {
		...
		if (omitHeadings.length > 0) {
			let omitRegex = new RegExp(
				`^h[1-6]\\. (${omitHeadings.map((h) => escapeRegex(h)).join("|")})\\n?\\n*`,
				"gm",
			);
	...
	async exportFileToJira(file: TFile) {
		...
		new Notice(
			`Exported to Jira format! Check ${outputPath}/${outputName}`,
		);
	}
	...
	async loadSettings() {
		...
		// Normalize arrays to avoid undefined/null issues
		this.settings.ignoreSections = this.settings.ignoreSections ?? [];
		this.settings.omitHeadings = this.settings.omitHeadings ?? [];
	}
